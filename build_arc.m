...
%BUILD_ARC Returns an arc of the passed radius, with the passed extreme
%points
...
% with vector out of the screen
centreline_direction = cross([start_end; 0], [0; 0; -1]);
centreline_direction = centreline_direction(1:2);
centreline_direction = centreline_direction / norm(centreline_direction);

distance_along_centreline = sqrt(radius^2 - (1/4) * distance_start_end^2);
...
% if the centre is above the point in the y-axis, the angle needs flipping,
...
% actually produce the arc's points
